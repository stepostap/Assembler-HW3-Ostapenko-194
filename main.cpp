...
/**
 * Для все чисел в промежутке [lower; upper] выполняется функция Fits
...
int main() {
    ...
    for (int i = 1; i < NumOfThreads; i++) {
        if (i == NumOfThreads - 1)
            Threads.push_back(thread(Func, (int)floor(max/NumOfThreads) * i, max, n, ref(Results)));
        else
            Threads.push_back(thread(Func, (int)floor(max/NumOfThreads) * i,
